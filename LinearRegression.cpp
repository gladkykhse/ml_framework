...
void LinearRegression<T>::fit(Matrix<T> &data,
    ...
    weights = Utils<T>::uniformMatrix(number_of_features, 1, 0, 1);
    ...
    for (int epoch = 1; epoch < epochs + 1; epoch++) {
        for (int i = 0; i < number_of_samples; i += batch_size) {
            ...
            for (int j = i; j < (i + batch_size); j++) {
                ...
                gradient = ((matrix_sample * (Math<T>::matmul(matrix_sample, weights).get_ijth(0, 0) - target)) *
                            (1.0 / batch_size)) + gradient;
            }
